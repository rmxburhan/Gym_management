...
import {
    validateFilterAttendancesStats,
    validatePostCheckIn,
...
route.get('/stats', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const { error, value } = validateFilterAttendancesStats.validate(
            req.query
        );
        if (error) throw error;
        const { startDate, endDate } = value;
        const datas = await attendanceService.getStats(startDate, endDate);
        return res.status(200).json({
            message: 'Stats data ',
            data: datas,
        });
...
route.get('/count', async (req: Request, res: Response, next: NextFunction) => {
    try {
        const datas = await attendanceService.getCount();
        return res.status(200).json({
        ...
        try {
            ...
            if (!code) {
                const error = new Error(
                    'Code not found. you can create manually'
                );
                error.name = 'NotFound';
                throw error;
            }
...
route.post(
    ...
    async (req: Request, res: Response, next: NextFunction) => {
        try {
            ...
            const attendance = await attendanceService.postCheckIn(
                user.id,
                code
            );
