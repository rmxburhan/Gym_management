...
const TrainerForm: FC<{ trainer: Trainer | null }> = ({ trainer }) => {
            ...
            updateTrainer(formData)
                .then((response) => {
                    if (response.status === 200) {
                        alert('Update trainer success');
                    } else {
                        alert('Update trainer failed');
                    }
                })
                .catch((err) => console.error(err));
        }
    ...
    useEffect(() => {
        if (trainer) {
            ...
            setValue(
                ...
                trainer.trainerDetail.identificationNumber
            );
            setValue('phoneNumber', trainer.trainerDetail.phoneNumber);
            setValue('bank', trainer.trainerDetail.bank);
            setValue('bankNumber', trainer.trainerDetail.bankNumber);
            remove();
            for (const address of trainer.trainerDetail.address) {
                append({
                    city: address.city,
                    state: address.state,
                    zip: address.zip,
    ...
    return (
        <div className="bg-white max-w-[600px] rounded-xl border p-4">
            ...
            <form className="grid gap-1.5" onSubmit={handleSubmit(onSubmit)}>
                ...
                <Button type="submit">{trainer ? 'Update' : 'Save'}</Button>
            </form>
        </div>
    );
};
